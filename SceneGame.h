...
public:
	...
	std::unique_ptr<sprite_batch> sprite_batches[5];
	...
	DirectX::XMFLOAT3 camera_focus = { 0.0f, 0.343f, -10.0f };
	...
	DirectX::XMFLOAT3 cheese_default_half_extents{ 0.035f, 0.006f, 0.048f };
	...
	std::vector<Vegetable> vegetables; // 野菜リスト

	btBoxShape* m_vegetableShape = nullptr;

	DirectX::XMFLOAT3 vegetable_default_half_extents{ 0.035f, 0.01f, 0.038f };
	float vegetable_default_mass = 0.5f;
	float vegetable_default_restitution = 0.0f;

	void add_vegetable(const DirectX::XMFLOAT3& pos,
		const DirectX::XMFLOAT3& half_extents,
		float mass,
		float restitution);

	void CheckKusiVegetableCollision();
	...
	DirectX::XMFLOAT3 pate_half_extents{ 0.08f, 0.003f, 0.01f }; // X=幅/2, Y=厚み/2, Z=奥行/2
	...
	float patty_gravity_y = -0.55f; // 下向き重力（単位系に応じて微調整）
	...
	static int CheckMenu(const int* a, const int* b, int size);

	static bool IsIntersectAABB(
